...
describe("ProfileForm", () => {
	...
	it("shows validation error for display name too long", () => {
		...
		expect(
			screen.getByText("Display name must be 50 characters or less"),
		).toBeInTheDocument();
	});
	...
	it("shows validation error for bio too long", () => {
		...
		expect(
			screen.getByText("Bio must be 500 characters or less"),
		).toBeInTheDocument();
	});
	...
	it("shows validation error for invalid avatar URL", () => {
		...
		expect(
			screen.getByText("Avatar URL must be a valid https URL"),
		).toBeInTheDocument();
	});
