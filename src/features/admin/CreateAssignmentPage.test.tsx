...
describe("CreateAssignmentPage", () => {
  ...
  beforeEach(() => {
    ...
    mockCreateAssignment.mockReturnValue({
      unwrap: vi.fn().mockResolvedValue({ assignmentId: "new-123", jobId: "job-123" }),
    });
    vi.mocked(useCreateAssignmentMutation).mockReturnValue([mockCreateAssignment, { isLoading: false }]);
  });
  ...
  it("renders create assignment form", () => {
    ...
    expect(screen.getByRole("heading", { name: "Create Assignment" })).toBeTruthy();
    expect(screen.getByText("Add a new SQL assignment for students")).toBeTruthy();
  ...
  it("calls createAssignment mutation with form data on valid submit", async () => {
    ...
    fireEvent.change(screen.getByLabelText("Title"), { target: { value: "Test Assignment" } });
    fireEvent.change(screen.getByLabelText("Description"), { target: { value: "Test description" } });
    fireEvent.change(screen.getByLabelText("Difficulty"), { target: { value: "medium" } });
    fireEvent.change(screen.getByLabelText("Mode"), { target: { value: "write" } });
    fireEvent.change(screen.getByLabelText("Sample Input"), { target: { value: "input1\ninput2" } });
    fireEvent.change(screen.getByLabelText("Sample Output"), { target: { value: '{"id": 1}' } });
    fireEvent.change(screen.getByLabelText("Init SQL"), { target: { value: "CREATE TABLE test (id INT);" } });
  ...
  it("shows server error when mutation fails", async () => {
    ...
    fireEvent.change(screen.getByLabelText("Sample Input"), { target: { value: "input" } });
    fireEvent.change(screen.getByLabelText("Sample Output"), { target: { value: "output" } });
    fireEvent.change(screen.getByLabelText("Init SQL"), { target: { value: "CREATE TABLE t (id INT);" } });
