...
describe("executionSlice", () => {
	...
	it("handles executionStarted action", async () => {
		const { default: executionReducer, executionStarted } = await import(
			"./executionSlice"
		);
		const initialState = {
			phase: "idle",
		...
		const state: any = executionReducer(
			initialState,
			executionStarted("task-123"),
		);
	...
	it("handles executionCompleted action", async () => {
		const { default: executionReducer, executionCompleted } = await import(
			"./executionSlice"
		);
		const initialState = {
			phase: "polling",
	...
	it("handles executionFailed action", async () => {
		const { default: executionReducer, executionFailed } = await import(
			"./executionSlice"
		);
		const initialState = {
			phase: "polling",
	...
	it("handles resetExecution action", async () => {
		const { default: executionReducer, resetExecution } = await import(
			"./executionSlice"
		);
		const initialState = {
			phase: "done",
